...
public class MemberService {
	...
	public Member save(Member member){
		...
		try {
			...
			if(temp != null){
				member.setId(temp.intValue());//将返回的主键值设置为id
			}
	...
	public void update(Member member){
		String sql = "update member  set mobile=?,pwd=?,real_name=?,nick_name=?,"
				+ "email=?,gender=?,register_time=?,role? where id=?";
		
		Object[] params = { member.getMobile(), member.getPwd(), 
				member.getReal_name(), member.getNick_name(), member.getEmail(), 
				member.isGender(), member.getRegister_time(), member.getRole(),member.getId()
		};
	...
	public Member findOne(Integer id){
		...
		//id,mobile,pwd,real_name,nick_name,email,gender,register_time,role
